...
from models.MR.divergence import JSDivergence
from utils.federated.trainer import FederatedTrainer


class FedNAP(GeneralRecommender):

    def __init__(self, config, dataloader):
        super(FedNAP, self).__init__(config, dataloader)
        self.config = config
    ...
    def forward(self, item_indices, txt_embed, vision_embed):
        ...
        fuse_embed = item_commonality + txt + vision

        return rating, item_personality, fuse_embed
...
class FedNAPTrainer(FederatedTrainer):

    def __init__(self, config, model, mg=False):
        super(FedNAPTrainer, self).__init__(config, model, mg)
        ...
        self.crit, self.independency, self.reg = nn.BCELoss(), JSDivergence(), nn.L1Loss()
    ...
    def _store_client_model(self, *args, **kwargs):
        ...
        for key in user_dict.keys():
            if any(sub in key for sub in ['item_commonality', 'mlp', 'attention', 'router', 'gate']):
    ...
    def _aggregate_params(self, *args, **kwargs):
        ...
        for user, params in participant_params.items():
            ...
            # 累加id_embed权重
            if 'item_commonality.weight' in params:
                if id_embed_weight_sum is None:
                    id_embed_weight_sum = params['item_commonality.weight'].data.clone()
                else:
                    id_embed_weight_sum += params['item_commonality.weight'].data.clone()
        ...
        self.item_commonality.weight.data = id_embed_weight_sum.clone() / num_participants
