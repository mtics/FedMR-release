...
class TrainDataLoader(AbstractDataLoader):
    ...
    def _get_full_uids_sample(self):
        ...
        user_tensor = torch.tensor(self.all_uids[self.pr: self.pr + self.step]).type(torch.LongTensor).to(
            self.device)
        self.pr += self.step
...
class FederatedDataLoader(object):
    ...
    def __init__(self, config, dataset, batch_size=1, neg_sampling=False, shuffle=False, stage='train',
        ...
        self.user_set = set(self.df[self.uid_field].unique())
        self.item_set = set(self.df[self.dataset.iid_field].unique())

        self.loaders = self._get_federated_loader()

    def _get_federated_loader(self):
        data_loader = None
        if self.stage == 'train':
            data_loader = self._get_train_loader()
        elif self.stage == 'valid' or self.stage == 'test':
            data_loader = self._get_eval_loader()

        return data_loader

    def _construct_user_dataset(self, uid):
        user_data = self.df[self.df[self.uid_field] == uid]
        user_dataset = RecDataset(self.config, user_data)
        user_dataset.user_num = 1
        user_dataset.item_num = self.dataset.item_num
        user_dataset.inter_num = len(user_data)
        return user_dataset

    def _get_train_loader(self):
        user_datasets, user_loader = {}, {}
        for user_id in self.user_set:
            user_datasets[user_id] = self._construct_user_dataset(user_id)
            user_loader[user_id] = TrainDataLoader(self.config, user_datasets[user_id],
                                                   batch_size=self.config['train_batch_size'],
                                                   shuffle=self.shuffle)
            user_loader[user_id].all_items = list(self.item_set)
            user_loader[user_id].all_items_set = self.item_set
    ...
    def _get_eval_loader(self):
        ...
        user_datasets, user_loader = {}, {}
        for user_id in self.user_set:
            # user_additional_data = self.additional_dataset[user_id]
            user_additional_data = self.additional_dataset
            user_datasets[user_id] = self._construct_user_dataset(user_id)
            user_loader[user_id] = EvalDataLoader(self.config, user_datasets[user_id],
